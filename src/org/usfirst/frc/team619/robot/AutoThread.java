...
public class AutoThread extends RobotThread{
	...
	public AutoThread(int period, ThreadManager threadManager, WheelDrive backRight, WheelDrive backLeft, WheelDrive frontRight, WheelDrive frontLeft, Lift l, Intake i, LimitSwitch[] aSwitches, AnalogUltrasonic[] anasonics)
	{
		...
		//Friendly Switch, Friendly Scale, Enemy Switch
		gameData = "RLR";
	...
	public void startAuto()
	{
		...
		if (autoType[2])
		{
			if ((gameData.charAt(0) == 'R') && (autoType[1]))
			{
				//Right Scale Right Start
				move(0, 0.3, 1000);
			}
			else if ((gameData.charAt(0) == 'R') && (autoType[0]))
			{
				//Right Scale Left Start
				move(0, 0.3 , 1000);
			}
			else if ((gameData.charAt(0) == 'L') && (autoType[0]))
			{
				// Left Scale Left Start
				move(0, 0.3 , 1000);
			}
			else if ((gameData.charAt(0) == 'L') && (autoType[1]))
			{
				//Left Scale Right Start
				move(0, 0.3 , 1000);
			}
		}
		else if (autoType[3]) 
		{
			if ((gameData.charAt(1) == 'R') && (autoType[1]))
			{
				//Right Switch Right Start
				move(0, 0.3, 1000);
			}
			else if ((gameData.charAt(1) == 'R') && (autoType[0]))
			{
				//Right Switch Left Start
				move(0, 0.3 , 1000);
			}
			else if ((gameData.charAt(1) == 'L') && (autoType[0]))
			{
				//Left Switch Left Start
				move(0, 0.3 , 1000);
			}
			
			else if ((gameData.charAt(1) == 'L') && (autoType[1]))
			{
